...
class FrontendTests(StaticLiveServerTestCase):
    ...
    def test_product_page_switches_images_correctly(self):
        product = models.Products.objects.create(
            name="The Balabazingas",
            slug="balazin-baa",
            price=Decimal("100.00"),
        )
        for fname in ["cb1.jpg", "cb2.jpg", "cb3.jpg"]:
            with open("main/fixture/cb/ s" %fname, "rb") as f:
            ...
            self.selenium.get(
                ...
                % (
                    ...
                    reverse(
                        "product",
                        kwargs={"slug": "balazin-baa"},
                    ),
                )
            )
            current_image = self.slenium.find_element_by_css_selector(
